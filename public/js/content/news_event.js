$(document).ready(function() {
    ...
    $("#news-event-image-dropzone").dropzone({
        ...
        init: function () {
            if (news_event_images.length > 0) {
                for (const img of news_event_images) {
    ...
    const table = new DataTable('#DataTable', {
        ...
        responsive: true,
        serverSide: true,
        ajax: '/content-management/news-events-list',
        columns: [{
                data: 'plus-icon',
                name: 'plus-icon'
            },
     ...
     //submit create form
     $(document).on('submit', '#new_event_create_form', function(e) {
        e.preventDefault();
        ...
        ask_confirm().then(result => {
            if(result.isConfirmed) {
                let formData = new FormData($('#new_event_create_form')[0]);

                $.ajax({
                    url: "/content-management/news-events",
                    type: "POST",
                    data: formData,
                    processData: false,
                    contentType: false,
                    ...
                    success: function (res) {
                        console.log(res);
                        if (res == "success") {
                            window.location.href = "/content-management/news-events";
                        } else {
    ...
    //submit edit form
    $(document).on('submit', '#new_event_edit_form', function(e) {
        e.preventDefault();
        ...
        ask_confirm().then(result => {
            if(result.isConfirmed) {
                let formData = new FormData($('#new_event_edit_form')[0]);
                let id = $('#new_event_id').val();

                $.ajax({
                    url: "/content-management/update-news-events/"+id,
                    type: "POST",
                    data: formData,
                    processData: false,
                    contentType: false,
    ...
    $(document).on('click', '.delete-btn', function(e) {
        ...
        if(id) {
            ask_confirm('Are you sure to delete ?').then(result => {
                if(result.isConfirmed) {
                    $.ajax({
                        url: "/content-management/news-events/" + id,
                        type: "DELETE",
                        headers: {
                            "X-CSRF-TOKEN": $('meta[name="csrf-token"]').attr(
                                "content"
    ...
    const check_fields_validation = () => {
        ...
        let new_event_id = $('#new_event_id').val();

        let names = [
            new_event_id ? null : "image",
            "title",
            "date",
            "content"
        ];
