$(document).ready(function() {
    ...
    const table = new DataTable('#DataTable', {
        ...
        responsive: true,
        serverSide: true,
        ajax: '/content-management/image-slider-list',
        columns: [{
                data: 'plus-icon',
                name: 'plus-icon'
            },
     ...
     $(document).on('submit', '#slider_create_form', function(e) {
        ...
        ask_confirm().then(result => {
            if(result.isConfirmed) {
                ...
                $.ajax({
                    url: "/content-management/image-sliders",
                    type: "POST",
                    data: formData,
                    processData: false,
                    contentType: false,
                    ...
                    success: function (res) {
                        console.log(res);
                        if (res == "success") {
                            window.location.href = "/content-management/image-sliders";
                        } else {
    ...
    $(document).on('submit', '#slider_edit_form', function(e) {
        ...
        ask_confirm().then(result => {
            if(result.isConfirmed) {
                ...
                let id = $('#image_slider_id').val();

                $.ajax({
                    url: "/content-management/update-image-slider/"+id,
                    type: "POST",
                    data: formData,
                    processData: false,
                    contentType: false,
    ...
    $(document).on('click', '.delete-btn', function(e) {
        ...
        if(id) {
            ask_confirm('Are you sure to delete ?').then(result => {
                if(result.isConfirmed) {
                    $.ajax({
                        url: "/content-management/image-sliders/" + id,
                        type: "DELETE",
                        headers: {
                            "X-CSRF-TOKEN": $('meta[name="csrf-token"]').attr(
                                "content"
    ...
    const check_fields_validation = () => {
        ...
        let image_slider_id = $('#image_slider_id').val();

        let names = [
            image_slider_id ? null : "image",
            "image_order_no",
        ];
